  ...
  const messages = [
    {
      ...
      content: await buildSystemPrompt(userProfile, context),
    },
...
async function buildSystemPrompt(
  ...
  const knowledge = await getRelevantKnowledge(userProfile.id, userProfile.companyId);
 ...
 */
async function getRelevantKnowledge(userId: string, companyId: string, limit = 5) {
