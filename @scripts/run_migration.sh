...
# StratixV2 Migration Script Runner
# Created: 2025-09-23
# Description: Execute database migrations in correct order
...
run_migration() {
    log_info "Starting StratixV2 database migration..."
    echo "=========================================="
    
    # Initial schema
    run_script "$SCRIPT_DIR/init/001_initial_schema.sql" "Initial Schema Setup" || exit 1
    
    # Multitenant support
    run_script "$SCRIPT_DIR/migrations/002_add_multitenant_support.sql" "Multitenant Support" || exit 1
    
    # AI suggestions
    run_script "$SCRIPT_DIR/migrations/003_add_ai_suggestions.sql" "AI Suggestions" || exit 1
    
    # Seed data (optional)
    if [ "$1" = "--with-seed-data" ]; then
        log_info "Including seed data..."
        run_script "$SCRIPT_DIR/init/004_seed_sample_data.sql" "Sample Data Seeding" || log_warning "Seed data failed, continuing..."
    fi
...
# Help function
show_help() {
    echo "StratixV2 Migration Script Runner"
    echo ""
    echo "Usage: $0 [COMMAND] [OPTIONS]"
    echo ""
    echo "Commands:"
    echo "  migrate              Run the complete migration"
